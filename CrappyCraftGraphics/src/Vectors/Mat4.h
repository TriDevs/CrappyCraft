...
template <typename T>
inline Mat4<T>::Mat4(Vec4<Vec4<T>> matrix)
    : mMatrix(matrix)
{
}

template <typename T>
inline Mat4<T>::Mat4(Vec4<T> abcd, Vec4<T> efgh, Vec4<T> ijkl, Vec4<T> mnop)
    : mMatrix(abcd, efgh, ijkl, mnop)
{
}

template <typename T>
inline Mat4<T>::Mat4(T a, T b, T c, T d, T e, T f, T g, T h, T i, T j, T k, T l, T m, T n, T o, T p)
...
template <typename T>
inline Mat4<T> &Mat4<T>::operator+=(const Mat4<T> &other)
{
...
template <typename T>
inline Mat4<T> &Mat4<T>::operator*=(const T scalar)
{
    for (int i = 0; i < 4; i++)
        mMatrix[i] *= scalar;
    return *this;
...
template <typename T>
inline Mat4<T> Mat4<T>::operator*(const Mat4<T> &mat) const
{
    Mat4<T> m;
...
template <typename T>
inline Vec4<T> Mat4<T>::operator[](int i) const
{
    return mMatrix[i];
}
